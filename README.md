...
export OCP_INSTALL_VERSION='4.21.3'
```
...
* Output:
    ...
    INFO Successfully pulled OpenShift 4.21.3 release images required for installation 
    ...
    INFO Copy openshift-install from: assets/cache/4.21.3-x86_64/openshift-install 
    INFO Download openshift-install from: https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/4.21.3/openshift-install-linux.tar.gz 
    ```
...
* Create Base ISO from the Base RAW disk image
    ...
    Run:
    ```sh
    podman run --rm -it --privileged -v $APPLIANCE_ASSETS:/assets:Z $APPLIANCE_IMAGE build iso --target-device /dev/sda --post-script post_deployment_script.sh
    ```
...
- Mount the Base ISO on the Virtual Media (in this case CDROM) of all three Virtual Machines.
    ...
    Note: If you prefer, instead of using the Base ISO you can use the Base RAW image wich should been burned into a USB stick (not documented in this repo ).
...
- Screenshot: Disks are automatically cloned

  ![Clone Disk Master0](./images/disk-clone-master0.png)

Once the OS is cloned the Post Script will kick in and shutdown the VMs.
...
In a disconnected environment, you would typically set disableAllDefaultSources: true to prevent the catalog pods from trying to pull index images from registry.redhat.io (which they cannot reach), and then point OLM at your own mirrored CatalogSource instead.
In this case, however, the cluster is connected, so the default catalog sources are enabled to allow operators managed by OLM to be installed directly from the public Red Hat image registries.
...
mkdir -p /tmp/r
cd /tmp/r
/usr/local/bin/agent-gather -O > agent-gather.tar.xz
